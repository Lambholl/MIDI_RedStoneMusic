...
def group_notes_by_quarter(notes):
    ...
    for note in notes:
        t = note['start_time']
        # 计算四舍五入到最近的三十二分音符位置
        quotient, remainder = divmod(t, 60)
        if remainder >= 30:
            rounded_time = (quotient + 1) * 60
        else:
            rounded_time = quotient * 60
        
        # 计算四分音符位置和开始时间类型
        index = rounded_time // 60
        position = index % 8
        index //= 8
...
if __name__ == "__main__":
    midi_notes = parse_midi_file("夢の歩みを見上げて.mid")  # 替换为你的MIDI文件路径
